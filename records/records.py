...
class Records:
    """
    Returns a Records class instance with GBIF occurence records stored
    in a panda DataFrame for a queried taxon between a range of years.
    ...
    def sdf(self):
        """
        Return a copy of the current .df dataframe selecting only the four
        most generally relevant columns: speicies, year, country, and
        stateProvince. This is only meant for viewing and will raise a warning
        if you try to modify it since it is a copy, and thus you would be
        setting values on a selection of a selction. See pandas docs in the
        warning for details.
        """
    ...
    def _get_all_records(self):
        ...
        while 1:
            ...
            # get data as json list of dicts add to 'data' list
            idata = res.json()
            data += idata["results"]
...
class Epochs:
    """
    Returns an Epochs class instance that includes GBIF occurence records and
    stores an extra label with each query that includes the interval (epoch)
    during which those records were collected, and returns all records in a
    ...
    def __init__(self, q, start, end, epochsize, **kwargs):
        ...
        # if rdicts, then build dataframe, otherwize skip it
        if rdicts:
    ...
    def sdf(self):
        """
        Return a copy of the current .df dataframe selecting only the five
        most generally relevant columns: speicies, year, epoch, country, and
        stateProvince.
        ...
        a selection of a selction. See pandas docs in the warning for details.
        """
        return self.df[
    ...
    def simpsons_diversity(self, by):
        """
        Calculates simpson's diversity index: the probabilitythat any two
        sampled individuals are the same species. Enter a key for groupby as
        a list of single or multiple keys
